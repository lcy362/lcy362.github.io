...
abbrlink: 58527
cover: /img/58527.jpg
tldr: 设计模式只是工具，六大设计原则才是写出好代码的根本
faq:
  - Q: 设计模式学了不用会忘怎么办？
  - Q: 哪种设计模式最常用？
  - Q: 设计模式和设计原则哪个更重要？
...
原文地址: https://lichuanyang.top/posts/58527/

## 常见问题

### Q: 设计模式学了不用会忘怎么办？

这很正常，关键在于**学习方法要对**。不要试图一次性背下 23 种模式的定义和结构——这种做法必然学了就忘。正确的路线是：先理解 SOLID 等设计原则的思想，然后在实际项目中遇到代码设计问题时，翻翻设计模式中有没有现成的解决方案，有目标地用、持续地重构。用得多了，对应场景和方案的映射就自然内化了，不需要刻意记忆。

### Q: 哪种设计模式最常用？

创建型模式中**工厂模式**和**建造者模式**用得最多——几乎任何有一定复杂度的项目都能在依赖注入和对象构造中看到它们的身影。结构型模式中**代理模式**（如 AOP、RPC 框架）、**适配器模式**（兼容不同接口）和**装饰器模式**（如 Java IO 流）是最常见的。行为型模式中**策略模式**、**模板方法模式**和**责任链模式**在实际业务开发中出场率最高，因为它们直接对应"根据条件选择算法""定义流程骨架""链式处理请求"这三种高频场景。

### Q: 设计模式和设计原则哪个更重要？

设计原则更重要。原则是"为什么"，模式是"怎么做"。SOLID、KISS、DRY、迪米特法则等设计原则指导你判断代码质量的得失，是所有好代码的共同基础。设计模式只是这些原则在某些典型场景下的具体实现方案。理解了原则，即使不记得某个模式的具体实现，也能写出好代码；反过来只记模式不识原理，就容易生搬硬套。

### Q: 单例模式为什么变成了反模式？

主要问题在于：单例**隐藏了类之间的依赖关系**——调用方的代码直接依赖 `Singleton.getInstance()`，从接口上看不出它用了全局状态；**可测试性差**——单例无法被 Mock，全局状态可能导致测试之间互相影响；**不支持有参数的构造函数**——限制了灵活性。但这不意味单例完全不能用，在确实需要一个进程内唯一实例且不涉及复杂测试场景时，单例仍然是最简洁的选择。关键是理解它的代价，而不是迷信"单例一定不好"。
---
