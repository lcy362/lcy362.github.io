...
知乎没有提供官方的数据导出 API，这里我用 Playwright 做了浏览器自动化。脚本启动 Chromium 浏览器后，手动扫码或密码登录一次，之后自动遍历个人主页抓取所有回答、文章和想法，登录状态会持久化保存到本地。后续运行时通过 `--reuse` 参数静默执行，不需要再次登录。同步后的文件按内容类型分类存放，每次只增量抓取新内容，已有的不会重复处理。

**如何将微信读书笔记同步到 Obsidian？**

微信读书在这方面做得相当友好——它开放了 Agent API Gateway（`i.weread.qq.com/api/agent/gateway`），开发者只需申请一个 API Key 即可调用。我自己用的同步脚本会先调用 `/user/notebooks` 接口获取有笔记的书籍列表，然后对每本新书分别拉取划线和想法，按章节分组输出为规范的 Markdown 文件。输出的格式大致是：书名和作者作为标题，每章的划线以引用块形式列出（附带划线日期），笔记和想法附在对应原文下方。整个过程完全是增量的——脚本维护了一个已同步书籍 ID 的状态文件，每次运行只处理新增的书籍，已有的自动跳过。151 本书的笔记就这样悄无声息地流入了 Obsidian，成为了知识中枢最丰富的一批原料。
...
这里有必要解释一下"知识库"和"人格蒸馏"的区别——它们共享同一套原始素材，但目标和产出完全不同。

**知识库（Wiki）回答的是"我知道什么"**。它从你的笔记、博客和读书摘录中提取客观知识，生成概念页面（比如"分布式一致性"）、实体页面（比如"Raft 算法"）、来源摘要页面（比如"《数据密集型应用设计》读书笔记"），并在它们之间建立密集的交叉引用。Wiki 层的目标是让知识变得可查询、可复用，像一个外部化的第二大脑。

**人格蒸馏（Personal Model）回答的是"我是谁"**。它从你的创作和阅读中反向推导出认知模式、表达风格和价值取向。比如，通过分析你写的技术博客，可以归纳出"论点先行、案例驱动、面向中高级读者"的表达风格；通过分析你的知乎回答，可以发现"第一性原理还原"和"量化思维"这两个反复出现的认知特征。人格蒸馏的产出不是知识条目，而是一个人的认知图谱——你擅长什么、怎么思考问题、看重什么价值。

两者在流程上高度相似——都有摄入、查询、一致性检查（Lint）等操作——但一个是向外看，结构化和整理你拥有的知识；一个是向内看，蒸馏和建模你作为个体的认知特征。这种"一体两面"的设计，是我觉得整个系统最有意思的地方。
